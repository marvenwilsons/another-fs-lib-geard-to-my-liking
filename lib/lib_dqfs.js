...
const d = (() => {
    ...
    lib.cp = (origin, dist) => {
        ...
        origin == '*' && ref.root == undefined && ref.errors.err('(fileSystem) cp: Select all "*": root path is not defined')
        const copy_case_1 = () => {
    ...
    lib.mv = (frm,to) => {
        ...
        fs.existsSync(frm) ? (frm = fspath.parse(frm) ) : err(ERRORS.NotFoundError('mv', frm))
        fs.existsSync(fspath.parse(to).dir) ? (to = fspath.parse(to) ) : err(ERRORS.NotFoundError('mv', fspath.parse(to).dir))


        // case rename
        frm.dir == to.dir && (() => {
            console.log('simple ranme')
        })()

        //case move to other directory but same name
        frm.dir != to.dir, frm.file == to.file && (() => {
            console.log('move to other dir')
        })()

        // case move to other directory with def name
        frm.dir != to.dir, frm.file != to.file && (() => {
            console.log('move to other dir')
        })()
    }
