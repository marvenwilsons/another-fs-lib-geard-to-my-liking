...
// libs
    ...
    lib.cp = (origin,dist) => {
        ...
        // sanitizing origin`
        const unCompleteArgs = () => err(`ERROR_cp: cp parameter cannot be undefined an empty string or a type of number`)
        ...
        origin == '*' && ref.root == undefined && err('ERROR_cp: Select all "*" can only work if the root directory is set, please use cd')        
        const c1 = () => {
            ...
            ref.functions.recursiveTraversDir(ref.root,(files,dir,timesExecuted) => {
                t(dir).map(e => {
                    ...
                    e.type == "folder/directory" && ref.fs.mkdirSync(rp);
