...
// libs
    ...
    lib.cp = (origin,dist) => {
        ...
        // case 1
        origin == '*' && dependency.root == undefined && err('ERROR_cp: Select all "*" can only work if the root directory is set, please use cd')        
        const c1 = () => {
            
            // check if path exist
            !dependency.fs.existsSync(dependency.root) && dependency.errors.PathDoesNotExistError('cp',dependency.root)
            ...
            dependency.functions.recursiveTraversDir(dependency.root,(files,dir,timesExecuted) => {
                t(dir).map(e => {
                    const rp = `${dist}/${e.publicPath.replace(dependency.root, "")}`
                    e.type == "folder/directory" && fs.mkdirSync(rp);
                    e.type == "file/application" && fs.writeFileSync(rp, fs.readFileSync(e.publicPath), "utf-8")
                })
            })
        }
        dependency.root != undefined && origin == '*' && c1()
