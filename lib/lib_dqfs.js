...
// libs
    ...
    lib.cp = (origin,dist) => {
        ...
        // sanitizing origin`
        const unCompleteArgs = () => ref.errors.err(`ERROR_cp: cp parameter cannot be undefined an empty string or a type of number`)
        ...
        origin == '*' && ref.root == undefined && ref.errors.err('ERROR_cp: Select all "*" can only work if the root directory is set, please use cd')        
        const c1 = () => {
        ...
        ref.root != undefined && origin == '*' && c1()

        // case 2 copying all contents inside origin into dist directory but cd is not define
        const c2 = () => {
            console.log(`c2 ${origin}`)
            const originPath = origin.replace('/*','/')
            ref.root = originPath
            c1()
        }
        typeof origin == 'string' && origin.endsWith('/*') && c2()

        // case 3 copying a directory into a directory
        const c3 = () => {
            !ref.fs.lstatSync(origin).isDirectory() && err("ERROR_cp: "+origin+" is not a directory")
            console.log(`c3 ${origin}`)
            //get dir name
            const dirName = Object.keys(queryString.parse(origin, "/"))[ Object.keys(queryString.parse(origin, "/")).length - 1 ]
            
            // check if directory exist
            ref.fs.readdirSync(dist).indexOf(dirName) != -1 && err("ERROR_cp: "+dirName+" already exist in "+dist+" directory ")
            ...
            ref.functions.recursiveTraversDir(origin, (files,dir,timesExecuted) => {
                t(dir).map(e => {
                    const contents = `${dist}/${e.publicPath.replace(ref.root, "")}`
                    // e.type == "folder/directory" && ref.fs.mkdirSync(contents)
                    // e.type == "file/application" && ref.fs.writeFileSync(contents, fs.readFileSync(e.publicPath), "utf-8")
                })
            })

        }
        typeof origin == 'string' && !origin.endsWith('/*') && origin != '*' && c3()
