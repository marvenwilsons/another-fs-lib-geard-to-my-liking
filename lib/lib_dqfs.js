...
// libs
    ...
    lib.cp = (origin,dist) => {
        ...
        // case 1
        origin == '*' && lib.rootPath == undefined && err('ERROR_cp: Select all "*" can only work if the root directory is set, please use cd')        
        const c1 = () => {
            !fs.existsSync(lib.rootPath) && PathDoesNotExistError('cp',lib.rootPath)
        }
        lib.rootPath != undefined && origin == '*' && c1()
