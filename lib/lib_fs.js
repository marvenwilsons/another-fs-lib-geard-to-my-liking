...
lib.dirCopy = (path1,path2) => {
    ...
    if(similarDir != undefined){
       ...
       // Start recursion
       const copyFiles = (p1,p2) => {
            let recursivePath2 = ''
            let recursivePath1 = p1
            let recursivePath3 = path1
            let rootPath2 = `${lib.path(path2)}/${path1}/`
            // set path2
            lib.dirList(recursivePath1).forEach(el => {
                let dist = rootPath2.concat(el)
                if(fs.lstatSync(`${recursivePath1}/${el}`).isDirectory()){
                    fs.mkdirSync(dist)
                    recursivePath2 = dist
                    recursivePath1 = lib.path(`${path1}/${el}`)
                }else{
                    lib.setRootDir = undefined
                    lib.fileCreate(dist)
                }
            })
            copyFiles(recursivePath1,recursivePath2)
       }
       copyFiles(lib.path(path1),lib.path(path2))
    }
