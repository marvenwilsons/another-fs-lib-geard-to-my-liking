...
lib.dirCopy = (path1,path2) => {
    ...
    r.layer('setting selectedItems', (data) => {
        ...
        const firstPathAddressContents = lib.dirList(firstPathAddress)
        selectedItems.items = []
    ...
    r.layer('look for similar name in dist folder and create',(data) => {
        const contentsOf2ndPath = lib.dirList(data.data.secondPathRootAddress)
    ...
    if(similarDir != undefined){
        ...
        const getLInks = (p1) => {
            ...
            const list = lib.dirList(lib.path(recursivePath))
