...
const layer = require('./lib_layers')
...
// @TODO: when attemp directory delete it crashesh
lib.fileDelete = (path) => {
    const accept = lib.checktype([
        [typeof path] == 'string'
    ])
    if(accept){
        if(fs.existsSync(lib.path(path))){
            if(!fs.lstatSync(lib.path(path)).isDirectory()){
                ...
                return lib.reject({
                    status: false,
                    message: `${path} is a directory, use dirDelete() instead`
                })
            }
        }else{
            return lib.reject({
...
lib.fileContentCopy = (_from,to,type) => {
    ...
    if(accept){
        if(fs.existsSync(lib.path(_from))){
            if(!fs.existsSync(lib.path(to))){
                fs.writeFileSync(
                    lib.path(to),
                    lib.fileRead(lib.path(_from),type),
                    type == undefined || type == null ? 'utf-8': type)
                ...
                fs.writeFileSync(lib.path(to),lib.fileRead(lib.path(_from),type),type == undefined || type == null ? 'utf-8' : type)
                return lib.resolve({
                    status: true,
                    case: 2,
...
lib.dirCreate = (path) => {
    ...
    if(accept){
        if(fs.existsSync(lib.path(path))){
            ...
            fs.mkdirSync(lib.path(path), 0o776)
            if(fs.existsSync(lib.path(path))){
                return lib.resolve({
                    status: true,
...
lib.dirDelete = (path) => {
    ...
    if(fs.readdirSync(lib.path(path)).length == 0){
        fs.rmdirSync(lib.path(path))
        return lib.resolve({status:true})
    } else {
        fileList = fs.readdirSync(lib.path(path))
        const root = lib.path(path)    
        const recursivelyScanFiles = (path) =>{
            lib.setRootDir = undefined
            let recursivePath = ''
            let folderContentsList = fs.readdirSync(lib.path(path))
            ...
            for(let i = 0; i < arrayLen; i++){            
                ...
                if(fs.readdirSync(lib.path(path)).length != 0){
                    recursivelyScanFiles(recursivePath)
                }else{
                    fs.rmdirSync(root)
                }    
            }
        }
        recursivelyScanFiles(lib.path(path))
        return lib.resolve({
            status: true,
...
//done
lib.dirRename = (oldname,newname) => {
    lib.checktype([
        [typeof newname] == 'string',
        [typeof oldname] == 'string'
    ])

    if(fs.existsSync(lib.path(oldname))){
        fs.renameSync(lib.path(oldname),lib.path(newname))
        if(fs.existsSync(lib.path(newname))){
            return lib.resolve({
                status: true,
        ...
        return lib.reject({
            status: false,
            message: `no such directory ${lib.path(oldname)}`
        })
    }

}
lib.dirCopy = (path1,path2) => {
    ...
    let selectedItems = {}
    let similarDir = undefined
    
    r.layer('setting selectedItems', (data) => {
        // well use later to stop the recursive function
        const firstPath = data.data.firstPath
        const firstPathAddress = lib.path(firstPath)
        
        //get contents of firstpath
        const firstPathAddressContents = fs.readdirSync(firstPathAddress)
        selectedItems.items = []
        firstPathAddressContents.forEach((el) => {
            const selections = `${firstPathAddress}/${el}`
            const sizeOfEachSelections = fs.lstatSync(selections).size
            
            // Assign values to selectedItems
            selectedItems[selections] = sizeOfEachSelections
            selectedItems.items.push(el)
        })

        let total = 0

        for(let prop in selectedItems) {
            total += selectedItems[prop]
        }
        selectedItems.totalSize = total

        r.next({
            fisrtPathContents : firstPathAddressContents,
            secondPathRootAddress : `${lib.path(data.data.secondPath)}/`,
            firstPathRootAddress: `${firstPathAddress}/`
        })
    })
    let recursionData = {}
    r.layer('look for similar name in dist folder and create',(data) => {
        const contentsOf2ndPath = fs.readdirSync(data.data.secondPathRootAddress)
        if(contentsOf2ndPath.indexOf(path1) != -1){
            similarDir = path1
        }else{
            fs.mkdirSync(`${data.data.secondPathRootAddress}/${path1}`)
        }

        recursionData = data.data        
    })
    let res = {}
    if(similarDir != undefined){
        return lib.reject({
            status: false,
            message: `Operation aborted ${path1} directory already exist in ${path2} directory`,
        })
    }else{
        let timesExecuted = 0
        let directoryLinksArray = []
        const getLInks = (p1) => {
            timesExecuted++
            let recursivePath = p1
            const list = fs.readdirSync(lib.path(recursivePath))
            for(let i = 0; i < list.length; i++){
                if(fs.lstatSync(lib.path(`${recursivePath}/${list[i]}`)).isDirectory()){
                    directoryLinksArray.push(`${recursivePath}/${Array(list[i])}`)
                    fs.mkdirSync(lib.path( `${path2}/${recursivePath}/${list[i]}` ))
                }else{
                    fs.writeFileSync(lib.path(`${path2}/${recursivePath}/${list[i]}`),
                    fs.readFileSync( lib.path(`${recursivePath}/${list[i]}`) ),
                    "utf-8"
                    )
                }
            }
            const stop = directoryLinksArray[timesExecuted - 1] == undefined            
            if(stop == false){
                getLInks(directoryLinksArray[timesExecuted - 1])
                directoryLinksArray = []
            }else{
                res = {
                    status: true,
                    message: `successfully copied ${path1} directory to ${path2} directory`
                }
            }
       }       
        getLInks(path1)
    }
    return lib.resolve(res)
}
// done
lib.dirMove = (oldpath,newpath) => {
    lib.checktype([
        [typeof newpath] == 'string',
        [typeof oldpath] == 'string'
    ],'dirMove')

    if(fs.existsSync(lib.path(oldpath))){
        fs.renameSync(lib.path(oldpath),lib.path(`${newpath}/${oldpath}`))
        if(fs.existsSync(lib.path(newpath))){
            return lib.resolve({
                status: true,
                message: `${oldpath} successfully moved to ${newpath}`
            })
        }
    }else {
        return lib.reject({
            status: false,
            message: `no such directory ${lib.path(oldpath)}`
        })
    }
